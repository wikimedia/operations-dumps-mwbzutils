...
void usage(char *message) {
  char * help =
"Usage: getlastrevidinbz2xml --filename file [--verbose]\n"
"       [--help] [--version]\n\n"
"Show the last rev in the specified MediaWiki XML dump file.\n"
...
"Report bugs in getlastrevidinbz2xml to <https://phabricator.wikimedia.org/>.\n\n"
...
"with this program.  If not, see <http://www.gnu.org/licenses/>\n\n"
  ...
  fprintf(stderr,"getlastrevidinbz2xml %s\n", version_string);
  fprintf(stderr,"%s",copyright);
  exit(-1);
}

/*
 if any rev id is found, appropriate updates will be made to rinfo
 no updates are made to the buffer about consumed data, the caller
 is responsible
 */
void find_last_revid_in_buffer(buf_info_t *buffer, rev_info_t *rinfo,
			       bz_info_t *bfile, int verbose) {
  regmatch_t *match_rev_id;
  regex_t compiled_rev_id;

  char *rev_id = "<revision>\n[ ]+<id>([0-9]+)</id>\n";

  char *match_from ;

  regcomp(&compiled_rev_id, rev_id, REG_EXTENDED);
  match_rev_id = (regmatch_t *)malloc(sizeof(regmatch_t)*3);

  if (buffer_is_empty(buffer)) return;

  match_from = (char *)buffer->next_to_read;
  while (regexec(&compiled_rev_id, match_from, 3, match_rev_id, 0) == 0) {
    /* found one, yay */
    if (match_rev_id[1].rm_so >=0) {
        rinfo->rev_id = atoi((char *)(match_from +match_rev_id[1].rm_so));
        rinfo->position = bfile->block_start;
        rinfo->bits_shifted = bfile->bits_shifted;
	/* get ready to search rest of buffer */
	match_from += match_rev_id[0].rm_eo;
    }
    else {
      /* should never happen */
  ...
  free(match_rev_id);
  regfree(&compiled_rev_id);
  return;
}


void init_rinfo(rev_info_t *rinfo) {
  rinfo->bits_shifted = -1;
  rinfo->position = (off_t)-1;
  rinfo->rev_id = -1;
  return;
}

/* 
   get the last rev id after position in file 
   expect position to be the start of a bz2 block
   if a revid is found, the structure rinfo will be updated accordingly
   returns:
      1 if a revid found,
      0 if no revid found,
      -1 on error
*/
int get_last_rev_id_after_offset(int fin, rev_info_t *rinfo,
  ...
  init_rinfo(rinfo);
    ...
    while (get_buffer_of_uncompressed_data(b, fin, bfile, FORWARD) >= 0 && (! bfile -> eof)) {
      find_last_revid_in_buffer(b, rinfo, bfile, verbose);
      /* did we hit eof? then th-th-that's all folks */
      if (bfile->eof)
	break;

      /*
	We keep reading more buffers because we want the _last_ revid,
	not the first one
      */
	...
	/* dump contents of buffer except last KEEP chars,
	   ...
	   We keep that much in case somewhere near the end was a
	   revision tag or a rev id tag that got cut off in the middle.
	*/
    ...
    /* see what's left in the buffer after eof. maybe we got something good */
      ...
      free_buffer(b);
      free(b);
      if (rinfo->rev_id == -1) return 0; /* not found */
      else if (rinfo->rev_id > 0) return 1; /* found */
      else return(-1); /* error */
    }
    else {
...
int giveup(int fin) {
  fprintf(stderr,"Failed to find any rev ids in file, exiting\n");
  close(fin);
  exit(1);
}

int main(int argc, char **argv) {
  int fin, res, rev_id=0;
  ...
  while (!rev_id) {
    ...
    res = get_last_rev_id_after_offset(fin, &rinfo, &bfile, upto, verbose);
    if (res > 0) {
      rev_id = rinfo.rev_id;
    }
    else {
